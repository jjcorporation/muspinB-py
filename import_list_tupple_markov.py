...
class Participant_data:
    ...
    def data_Amb_call(self, path_file):
        data = pd.read_excel(
            'C:/Users/Utilisateur/Desktop/Graph par participant/2021_02_22_95/XLSX_Amb/Ok/' + path_file)
    ...
    def data_nAmb_call(self, path_file):
        data = pd.read_excel(
            'C:/Users/Utilisateur/Desktop/Graph par participant/2021_02_22_95/XLSX_nAmb/Ok/' + path_file)
        data = data .to_records(index=False)
        data = list(data)
        return data

    # Warning : may remove a lot for nAmb condition, plus besoin avec new initialisation
    def remove_one_or_solo(self, list_considérée, type_stim):
        touch_press = [x[0] for x in list_considérée]
        Number_touch_press = [
            [x, touch_press.count(x)] for x in set(touch_press)]
        Remove = []
        ...
        New_list = [list_considérée[i] for i in range(
            len(list_considérée)) if list_considérée[i][0] not in Remove]
        if type_stim == 'Amb':
            ...
            warnings.warn(
                "Warning : Is it an Amb or nAmb list that is injected into the loop ? Please Specify type 'Amb' or 'nAmb' in type_stim ")
...
Participant_name = 'P_1005'  # P_'+ sujet[0:4]
# Chose the participant, Ronald Help, c'est galère d'instancier un nom différent pour chaque objets
Participant = Participant_data()
# Create List
Participant.Amb_list = Participant.data_Amb_call('1005.xlsx')
Participant.nAmb_list = Participant.data_nAmb_call('1005.xlsx')
...
k = len(states)
tm = np.zeros((k, k))
# force donnée, soit disant observé 3 échantillons (moyenne_temps_séjours_a_priori), si observe 20 autre, moins de force, m
tm.flat[::k+1] = 1
Participant.mrp_Amb = MRP(states, tm=tm, mu=np.full((k, ), moyenne_temps_séjours_a_priori),  sigma=np.full((k,), écart_type_temps_séjours_a_priori))
 # if not s[0]== 'Left_coh'])
Participant.mrp_Amb.train([(s[0], s[2]) for s in Participant.Amb_list])
